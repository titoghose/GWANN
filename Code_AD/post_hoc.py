...
def mine_agora(exp_name:str, model_name:str):
    results_folder = f'results_{exp_name}_{model_name.split("_")[0]}'
...
def manhattan(label:str, exp_name:str, model_name:str, metric:str, p_cut_off:float=1e-30):
    results_folder = f'results_{exp_name}_{model_name.split("_")[0]}'
...
def hit_gene_win_snps(label:str, exp_name:str, model_name:str, metric:str, 
...
if __name__ == '__main__':
    for rseed in [0, 73, 347, 816]:
        label = 'FH_AD'
        exp_name = f'ArchTest_{rseed}{rseed}_GS20'
        model_name = 'GWANNet5_[32,16]_Dr_0.5_LR:0.005_BS:256_Optim:adam'
        print(exp_name)
        ...
        calculate_p_values(label=label, exp_name=exp_name, model_name=model_name,
                       metric='snp_grads', greater_is_better=True)
        # calculate_p_values(label=label, exp_name=exp_name, model_name=model_name,
