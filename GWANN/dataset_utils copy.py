...
class PGEN2Pandas():
    ...
    def get_samples(self) -> list:
        print(self.psam.head())
        return self.psam['IID'].to_list()

    def get_dosage_matrix(self, chrom:str, start:int, 
...
def group_data_prep(X:np.ndarray, y:np.ndarray, Xt:np.ndarray, yt:np.ndarray, 
                    grp_size:int, covs:list, test_size:int=0.0, 
                    train_oversample:int=10, test_oversample:int=10) -> tuple:
